...
function locatePlaywrightCli() {
  ...
  const skillWrapper = join(homedir(), ".codex", "skills", "playwright", "scripts", "playwright_cli.sh");
  if (existsSync(skillWrapper)) return skillWrapper;
  const lookup = spawnSync("sh", ["-c", "command -v playwright-cli"], { encoding: "utf8" });
  const path = lookup.stdout.trim();
  if (lookup.status === 0 && path) return path;
  throw new Error(
    "playwright-cli is unavailable; set CORNERFILL_PLAYWRIGHT_CLI or install @playwright/cli",
  );
}
