...
- visible borders are generated as SVG data-URI rings;
...
The remaining limitation is not rotation. It is the box semantics that Paint Level 1 never exposes.

## `border-shape` limitation

Painting one/two basic-shape paths is feasible. Full native semantics still include:
...
Cornerfill can add a useful painted `border-shape` subset, especially the PolyCSS triangle's two-shape fill form, without pretending the entire property is finished.
