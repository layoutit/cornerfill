...
`PaintRenderingContext2D` includes the Canvas compositing, path, and image-drawing mixins, so the same operation is available to native Paint Worklets and the main-thread fallback contexts. This is an internal painter operation, not a CSS mask.
...
As of this snapshot, the draft's forward algorithm says to compute `k = 0.5^abs(s)` and then `convexHalfCorner = 0.5^(1/k)`. That is not the inverse of the following conversion algorithm, reverses the stated limiting behavior, and differs from Blink and WebKit. The open [CSSWG issue 14157](https://github.com/w3c/csswg-drafts/issues/14157) calls out this inconsistency as well as a separate concave hull-direction mismatch.
...
The first shape is the outer boundary and defaults to the border box. The second is the inner boundary and defaults to the padding box. The border is the filled region between them.

### Interactions

- Non-`none` `border-shape` causes `border-radius` and `corner-shape` to be ignored.
- Outer shadow follows the outer path; inset shadow follows the inner path.
- The inner path is the native overflow clip.
- Layout and content flow remain rectangular.

The PolyCSS rule `polygon(50% 0, 100% 100%, 0 100%) circle(0)` is the simple two-shape fill case. A complete implementation is much larger because `<basic-shape>` includes circles, ellipses, inset/rect forms, polygons, `path()`, and `shape()` with geometry-box and percentage resolution.
...
Paint-mode conformance should pass the rendering subset. Overflow and hit-test tests must be marked “unimplementable by this backend,” not silently counted as passes.
